...
router.get('/api/getProduct', async (req, res) => {
  ...
  router.post('/api/getProductReOrderByEmail', async (req, res) => {
    ...
    let temp = req.body.data?.map( item => [item.product_id ,item.product_name ,item.stock ,item.reorder_level ,item.reorder_status,item.selling_price, item.buying_price,])
    try {
      ...
      doc.on("end", () => {
        ...
        const message = {
          ...
          to: 'gamathigeh@gmail.com',
          subject: "ByTech POS Product Report",
          attachments: [
            {
              filename: `Re-orderList.pdf`,
              content: pdfData,
        ...
        transporter.sendMail(message, (error, info) => {
          if (error) {
            ...
            console.log("Product Report sent successfully");
            res.status(200).send("Product Report sent successfully");
          }
        });
      });
      ...
      doc.fillColor('gray').fontSize(14).text('Date : ' + (new Date()).toDateString(), 50, 110);

      // Generate the table dynamically
      
      const table = [
        ['Product ID', 'Name', 'Current Stock', 'Re-order level', 'Status', 'Selling Price', 'Buying Price'],
      ];
