...
router.get('/api/getProduct', async (req, res) => {
  ...
  router.post('/api/getProductReOrderByEmail', async (req, res) => {
    ...
    let temp = req.body.data?.map( item => [item.product_id ,item.product_name ,item.stock ,item.reorder_level ,item.reorder_status])
    try {
  ...
  router.post('/api/getProductReOrderReport', async (req, res) => {
    console.log(req.body)
    let temp = req.body?.map( item => [item.product_id ,item.product_name ,item.stock ,item.reorder_level ,item.reorder_status])
    try {
      const doc = new PDFDocument();
      // Set the content type as application/pdf
      res.setHeader('Content-Type', 'application/pdf');
